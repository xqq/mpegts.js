...
| `liveBufferChasingOnPaused?`     | `boolean` | `false`                      | Chasing the live stream latency caused by the internal buffer in HTMLMediaElement even if HTMLMediaElement is paused. Effective only if `isLive: true` and `liveBufferLatencyChasing: true` |
